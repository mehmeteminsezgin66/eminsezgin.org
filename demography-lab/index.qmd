...
İki yıl seçin: toplam doğurganlık hızındaki değişimin hangi doğum sırasından (1., 2., 3., 4+. çocuk) kaynaklandığını görün.

```{ojs}
//| echo: false

yillar_secenekleri = ["2013","2014","2015","2016","2017","2018","2019","2020","2021","2022","2023","2024","2025"]

viewof yil_a = Inputs.select(yillar_secenekleri, {value: "2013", label: "Baslangic yili"})
viewof yil_b = Inputs.select(yillar_secenekleri, {value: "2025", label: "Bitis yili"})
```
...
tdh1_seri = [0.760,0.780,0.780,0.768,0.758,0.721,0.684,0.657,0.615,0.629,0.612,0.622,0.608]
tdh2_seri = [0.688,0.704,0.689,0.675,0.662,0.643,0.602,0.568,0.555,0.505,0.465,0.456,0.433]
tdh3_seri = [0.366,0.396,0.393,0.386,0.378,0.367,0.344,0.317,0.311,0.285,0.251,0.238,0.219]
tdh4_seri = [0.275,0.288,0.276,0.269,0.265,0.254,0.238,0.219,0.221,0.204,0.180,0.168,0.153]
...
html`<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 16px 0;">
  <div style="background: #f8f9fa; border-left: 4px solid #888; padding: 12px; border-radius: 4px;">
    ...
    <div style="font-size: 1.6em; font-weight: bold; color: #888;">${tdh_a_toplam.toFixed(3)}</div>
  </div>
  <div style="background: #f8f9fa; border-left: 4px solid #1F3864; padding: 12px; border-radius: 4px;">
    <div style="font-size: 0.75em; color: #666;">TDH ${yil_b}</div>
    <div style="font-size: 1.6em; font-weight: bold; color: #1F3864;">${tdh_b_toplam.toFixed(3)}</div>
  </div>
  <div style="background: ${fark_toplam < 0 ? '#ffebee' : '#e8f5e9'}; border-left: 4px solid ${fark_toplam < 0 ? '#B33A3A' : '#2E7D32'}; padding: 12px; border-radius: 4px;">
    <div style="font-size: 0.75em; color: #666;">Toplam Degisim</div>
    <div style="font-size: 1.6em; font-weight: bold; color: ${fark_toplam < 0 ? '#B33A3A' : '#2E7D32'};">${fark_toplam >= 0 ? '+' : ''}${fark_toplam.toFixed(3)}</div>
  </div>
</div>`
```
...
katki_veri = [
  {siralama: "1. Cocuk", deger: fark_1},
  {siralama: "2. Cocuk", deger: fark_2},
  {siralama: "3. Cocuk", deger: fark_3},
  {siralama: "4+ Cocuk", deger: fark_4}
]

Plot.plot({
  width: 700, height: 320,
  marginLeft: 90,
  x: {label: "TDH Degisimine Katki"},
  y: {label: null, domain: ["4+ Cocuk","3. Cocuk","2. Cocuk","1. Cocuk"], type: "band"},
  marks: [
    Plot.ruleX([0]),
    Plot.barX(katki_veri, {x: "deger", y: "siralama", fill: d => d.deger < 0 ? "#B33A3A" : "#2E7D32"}),
    Plot.text(katki_veri, {x: "deger", y: "siralama", text: d => d.deger.toFixed(3), dx: d => d.deger < 0 ? -22 : 22, fontSize: 11, fontWeight: "bold"})
  ]
})
```
...
Plot.plot({
  width: 700, height: 320,
  x: {label: null, domain: [yil_a, yil_b]},
  y: {label: "TDH Bileseni", domain: [0, 0.85]},
  color: {domain: ["1. Cocuk","2. Cocuk","3. Cocuk","4+ Cocuk"], range: ["#1F3864","#9E7A1A","#5B8C5A","#B33A3A"]},
  marks: [
    Plot.barY([
      {yil: yil_a, siralama: "1. Cocuk", deger: deger_a.cocuk1},
      {yil: yil_a, siralama: "2. Cocuk", deger: deger_a.cocuk2},
      {yil: yil_a, siralama: "3. Cocuk", deger: deger_a.cocuk3},
      {yil: yil_a, siralama: "4+ Cocuk", deger: deger_a.cocuk4},
      {yil: yil_b, siralama: "1. Cocuk", deger: deger_b.cocuk1},
      {yil: yil_b, siralama: "2. Cocuk", deger: deger_b.cocuk2},
      {yil: yil_b, siralama: "3. Cocuk", deger: deger_b.cocuk3},
      {yil: yil_b, siralama: "4+ Cocuk", deger: deger_b.cocuk4}
    ], {x: "yil", y: "deger", fill: "siralama"})
  ]
})
```

::: {.callout-important appearance="simple"}
**Kaynak:** TUIK Dogum Istatistikleri, dogum sirasina gore TDH bilesenleri (TDHₙ = TDH × (n. sira payi/100)), 2013-2025 tam seri. Varsayilan secimde (2013→2025), toplam TDH kaybinin (-0,676) **%37,7'si 2. cocuktan**, %22,5'i 1. cocuktan, %21,7'si 3. cocuktan ve %18,0'i 4+ cocuktan kaynaklanmaktadir — 2. cocuk (1→2 gecisi) tek basina en buyuk paya sahiptir. Farkli yil ciftleri secerek (orn. 2021 vs 2025) son donemdeki hizlanmayi da gozlemleyebilirsiniz.
:::
