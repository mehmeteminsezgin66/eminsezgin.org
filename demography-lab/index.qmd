...
Plot.plot({
  ...
  marks: [
    Plot.barY([{x: 0.3, y: ideal_secilen}], {x: "x", y: "y", fill: "#9E7A1A", width: 0.3}),
    Plot.barY([{x: 0.7, y: tdh_gerceklesen}], {x: "x", y: "y", fill: "#1F3864", width: 0.3}),
    Plot.text([{x: 0.3, y: ideal_secilen + 0.15}], {text: [`Ideal: ${ideal_secilen.toFixed(1)}`], fontSize: 12, fontWeight: "bold"}),
    Plot.text([{x: 0.7, y: tdh_gerceklesen + 0.15}], {text: [`Gerceklesen: ${tdh_gerceklesen.toFixed(2)}`], fontSize: 12, fontWeight: "bold"}),
    Plot.ruleY([2.1], {stroke: "#888", strokeDasharray: "4,4"}),
    Plot.text([{x: 0.95, y: 2.25}], {text: ["Yenilenme (2,1)"], fill: "#888", fontSize: 9, textAnchor: "end"})
  ]
})
```
